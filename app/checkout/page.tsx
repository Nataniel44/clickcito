...
import {
    ...
    Check, Plus, Clock, Wallet, QrCode, Banknote
} from "lucide-react";
...
export default function CheckoutPage() {
    ...
    const [entregaMetodo, setEntregaMetodo] = useState<"delivery" | "retiro" | "mesa">("delivery");
    ...
    if (authLoading) return <div className="h-screen flex items-center justify-center bg-white dark:bg-black"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-orange-500" /></div>;
        ...
        return (
            <div className="h-screen flex flex-col items-center justify-center p-6 bg-gray-50 dark:bg-black">
                <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] shadow-xl text-center max-w-sm border border-gray-100 dark:border-zinc-800">
                    <ShoppingBag className="text-orange-500 mx-auto mb-4" size={48} />
                    ...
                    <button onClick={() => router.push("/")} className="w-full py-3 bg-orange-600 text-white font-black rounded-xl text-sm">Explorar Tienda</button>
                </div>
            </div>
        );
    }
    ...
    const availableMethods = [
        { id: 'delivery', label: 'Delivery', icon: Truck, time: negocioInfo?.configuracion_logistica?.tiempo_aprox_delivery || '~45 min', enabled: negocioInfo?.configuracion_logistica?.delivery_habilitado ?? true },
        { id: 'retiro', label: 'Retiro', icon: Store, time: '~20 min', enabled: negocioInfo?.configuracion_logistica?.takeaway_habilitado ?? true },
        { id: 'mesa', label: 'Mesa', icon: Armchair, time: 'Salón', enabled: negocioInfo?.configuracion_logistica?.mesa_habilitado ?? false }
    ].filter(m => m.enabled);
    ...
    const handleConfirmarPedido = async () => {
        ...
        if (entregaMetodo === "delivery" && (!direccion || !telefono)) {
            toast.error("Datos de delivery incompletos");
            return;
        }
        if (entregaMetodo === "retiro" && (!nombreCliente || !telefono)) {
            toast.error("Datos de retiro incompletos");
            return;
        }
        ...
        try {
            ...
            const metadata = {
                ...
                direccion_envio: entregaMetodo === "delivery" ? direccion : "Retiro en Local/Mesa",
                nro_mesa: nroMesa || "",
                notas_cliente: notas || "",
        ...
        <div className="min-h-screen bg-[#FDFDFD] dark:bg-black pb-32">
            ...
            <main className="max-w-xl mx-auto p-4 space-y-4">
                ...
                <div className="bg-white dark:bg-zinc-950 rounded-[2.5rem] shadow-2xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-zinc-900 overflow-hidden">
                    ...
                    <div className="p-5">
                        <div className={`grid gap-2 mb-6 ${availableMethods.length === 3 ? 'grid-cols-3' : availableMethods.length === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                            {availableMethods.map((met) => (
                                ...
                                >
                                    ...
                                    <span className="text-[11px] font-black uppercase">{met.label}</span>
                                    <span className="text-[9px] opacity-60 font-bold">{met.time}</span>
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-12 pr-4 py-4 rounded-2xl bg-gray-100/50 dark:bg-zinc-900/50 border-none outline-none font-bold text-[13px] text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:bg-white dark:focus:bg-zinc-900 placeholder:text-gray-400"
                                            />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                ...
                                                className="w-full px-4 py-4 rounded-2xl bg-gray-100/50 dark:bg-zinc-900/50 border-none outline-none font-bold text-[13px] text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:bg-white dark:focus:bg-zinc-900 placeholder:text-gray-400"
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                            ...
                            {entregaMetodo === 'retiro' && (
                                <div className="space-y-4">
                                    <div className="p-4 rounded-2xl bg-gray-50 dark:bg-zinc-900/50 border border-gray-100 dark:border-zinc-800 flex items-start gap-4">
                                        ...
                                        <div>
                                            <p className="text-[13px] font-black text-gray-900 dark:text-white">{negocioInfo?.ubicacion || "Dirección del local"}</p>
                                            <p className="text-[11px] text-gray-400 font-bold uppercase mt-1 italic">Lun–Sáb 10:00 a 22:00 · Abierto ahora</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">¿A nombre de quién?</label>
                                            <input
                                                type="text"
                                                value={nombreCliente}
                                                onChange={(e) => setNombreCliente(e.target.value)}
                            ...
                            <div className="space-y-1.5 pt-2">
                                ...
                                <div className="relative">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full pl-12 pr-4 py-4 rounded-2xl bg-gray-100/50 dark:bg-zinc-900/50 border-none outline-none font-bold text-[13px] text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:bg-white dark:focus:bg-zinc-900 placeholder:text-gray-400 resize-none"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        {[
                            ...
                            <button
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-2xl border transition-all active:scale-[0.98] ${metodoPago === pago.id
                                    ? 'border-orange-500 bg-orange-50/20 dark:bg-orange-500/5'
                                    : 'border-gray-100 dark:border-zinc-900 bg-gray-50/50 dark:bg-zinc-900/50'}`}
                            >
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${metodoPago === pago.id ? 'border-orange-500' : 'border-gray-200 dark:border-zinc-800'}`}>
                                    {metodoPago === pago.id && <div className="w-3 h-3 rounded-full bg-orange-500 animate-in zoom-in" />}
                                </div>
                ...
                <div className="bg-white dark:bg-zinc-950 rounded-[2.5rem] shadow-2xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-zinc-900 overflow-hidden">
                    ...
                    <div className="p-5 space-y-3">
                        ...
                        <div className="pt-3 border-t border-gray-50 dark:border-zinc-900/30 flex justify-between items-center">
                            ...
                            <span className="text-2xl font-black text-orange-600">${cartTotal.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
            ...
            <footer className="fixed bottom-0 inset-x-0 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border-t border-gray-100 dark:border-zinc-900 p-4 z-50">
                <div className="max-w-xl mx-auto flex items-center gap-4">
                    ...
                    <button
                        ...
                        className="flex-[1.5] py-4 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white font-black rounded-2xl flex items-center justify-center gap-2 shadow-xl shadow-orange-600/20 active:scale-95 transition-all outline-none text-sm uppercase tracking-tight"
                    >
